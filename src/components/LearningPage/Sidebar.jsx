...
const Sidebar = ({
  ...
  const completedHours = Math.round((course.duration || 0) * (progress / 100));
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-3 gap-3 mb-4">
        <div className="bg-gray-700/40 rounded-xl p-3 text-center">
          ...
          <div className="text-xs text-gray-300">
            {t("learningPage.sidebar.progress")}
          </div>
        </div>
        <div className="bg-gray-700/40 rounded-xl p-3 text-center">
          <div className="font-semibold">
            {formatNumber(completedLectures.length)}/{formatNumber(lectures.length)}
          </div>
          <div className="text-xs text-gray-300">
            {t("learningPage.sidebar.lectures")}
          </div>
        </div>
        <div className="bg-gray-700/40 rounded-xl p-3 text-center">
          <div className="font-semibold">
            {t("learningPage.sidebar.hoursShort", {
              count: formatNumber(completedHours),
            })}
          </div>
          <div className="text-xs text-gray-300">
            {t("learningPage.sidebar.completed")}
          </div>
        </div>
      </div>
      ...
      <div
        ...
        style={{ maxHeight: isDesktop ? "calc(100vh - 320px)" : "auto" }}
      >
        <div className="flex items-center justify-between mb-2">
          <h4 className="text-xs text-gray-300 uppercase tracking-wide">
            {t("learningPage.sidebar.courseContent")}
          </h4>
          <span className="text-xs text-gray-400">
            {t("learningPage.sidebar.completedCount", {
          ...
          {lectures.map((lecture, idx) => {
            ...
            return (
              ...
              >
                ...
                >
                  {isDone ? <FiCheck /> : formatNumber(idx + 1)}
                </div>
                  ...
                  <div className="text-xs text-gray-400 mt-1 flex items-center gap-2 flex-wrap">
                    {hasVideo ? (
                      ...
                      <span className="text-gray-400 text-xs">
                        {t("learningPage.sidebar.noVideo")}
                      </span>
                    )}
                    ...
                    {lecture.resources?.length > 0 && (
                      <span className="bg-blue-600/20 px-2 py-0.5 rounded-md text-blue-300 text-xs flex items-center gap-1">
                        <FiDownload size={12} />
                        {formatNumber(lecture.resources.length)}
                      </span>
                    )}
      ...
      <div className="mt-4 space-y-2">
        ...
        >
          ...
          {refreshing
            ? t("learningPage.sidebar.refreshing")
            : t("learningPage.sidebar.refreshData")}
        </button>
        ...
        >
          <div className="flex items-center justify-center gap-2">
            <FiHome /> {t("learningPage.sidebar.backToCourses")}
          </div>
        </button>

        <div className="text-xs text-gray-400 text-center pt-2 border-t border-gray-700/30">
          {lastActivity
            ? t("learningPage.sidebar.lastUpdated", {
                date: formatDate(lastActivity),
              })
            : t("learningPage.sidebar.noActivity")}
        </div>
      </div>
    </motion.aside>
  );
